...
| `--format` | `text` | Output format: `text` or `json`. `json` prints one object on stdout: `queue`, `cutoff`, `statuses`, `task_rows`, `tick_rows` and `dry_run`. `queue` is `null` when no `--queue` is given, `cutoff` is ISO 8601, and `statuses` is a list. On database contention failure, the counts of rows already deleted in committed batches are printed before the same non-zero exit. |
